...
with gzip.open('../../datos_ceibal/datos_resto_del_anio/agosto/datos_filtrados_APs_Agosto.csv.gz', 'rt') as f:
    df_aps = pd.read_csv(f)
# elimino columnas que no interesan
local_n_mac = df_aps.drop(['Unnamed: 0', 'timestamp', 'MAC_AP',
...
local_n_mac['building_id'] = local_n_mac['MAC_AP_hexa'].copy()
...
local_n_mac.to_csv('./from_mac_hexa_to_building_id.csv', index=False)