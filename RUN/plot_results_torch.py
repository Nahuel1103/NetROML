...
def plot_results(building_id, b5g, tx_probs, channel_probs, power_probs, 
    ...
    of the system with hierarchical decision architecture and plots them.
    
    New parameters:
    - tx_probs, channel_probs, power_probs: Final probabilities from last batch
    - tx_policy_values, channel_policy_values, power_policy_values: Evolution of policies
    - num_channels, num_power_levels: Architecture parameters
    """
    ...
    # Save final policies as text files
    tx_final = torch.mean(tx_probs, dim=[0,1]).detach().numpy()
    channel_final = torch.mean(channel_probs, dim=[0,1]).detach().numpy()
    power_final = torch.mean(power_probs, dim=[0,1]).detach().numpy()
    
    # TX policy
    tx_path = os.path.join(path, 'final_tx_policy_' + eps_str + '_' + mu_lr_str + '_' + batch_size_str + '.txt')
    np.savetxt(tx_path, tx_final, delimiter=',', fmt='%.4f',
               header='no_tx,tx')
    
    # Channel policy
    channel_path = os.path.join(path, 'final_channel_policy_' + eps_str + '_' + mu_lr_str + '_' + batch_size_str + '.txt')
    np.savetxt(channel_path, channel_final, delimiter=',', fmt='%.4f',
               header=','.join([f'channel_{i}' for i in range(num_channels)]))
    
    # Power policy
    power_path = os.path.join(path, 'final_power_policy_' + eps_str + '_' + mu_lr_str + '_' + batch_size_str + '.txt')
    np.savetxt(power_path, power_final, delimiter=',', fmt='%.4f',
               header=','.join(power_labels))
