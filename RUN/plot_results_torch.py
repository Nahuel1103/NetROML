...
def plot_results(building_id, b5g, normalized_psi, normalized_psi_values=[], power_values=[], 
    ...
    if train:
        if mark:
            path = '/Users/mauriciovieirarodriguez/project/NetROML/results/' + str(band[b5g]) + '_' + str(building_id) + '/torch_results/n_layers' + str(num_layers) + '_order' + str(K) + '/mark_' + eps_str +  '_' + mu_lr_str + '_' + str(batch_size) + '_' + str(epochs) + '_' + str(rn) + '_' + str(rn1)
        else:
